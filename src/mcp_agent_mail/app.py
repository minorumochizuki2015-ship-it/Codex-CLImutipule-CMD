...
from .guard import (
    install_guard as install_guard_script,
    uninstall_guard as uninstall_guard_script,
)
...
TOOL_METRICS: defaultdict[str, dict[str, int]] = defaultdict(
    lambda: {"calls": 0, "errors": 0}
)
TOOL_CLUSTER_MAP: dict[str, str] = {}
TOOL_METADATA: dict[str, dict[str, Any]] = {}

RECENT_TOOL_USAGE: deque[tuple[datetime, str, Optional[str], Optional[str]]] = deque(
    maxlen=4096
)

AGENT_ALIAS_SEQUENCE: tuple[str, ...] = (
    "Alpha",
    "Beta",
    "Gamma",
    "Delta",
...
def _extract_argument(
    bound: inspect.BoundArguments, name: Optional[str]
) -> Optional[str]:
    if not name:
        return None
...
def _record_recent(
    tool_name: str, project: Optional[str], agent: Optional[str]
) -> None:
...
def _query_params_from_context(ctx: Context | None) -> dict[str, str]:
    if ctx is None:
        return {}
    try:
...
def _extract_agent_from_result(result: Any) -> Optional[str]:
    if isinstance(result, dict):
        for key in ("name", "agent_name", "sender_name"):
...
def _extract_project_from_result(result: Any) -> Optional[Union[str, int]]:
    if isinstance(result, dict):
        for key in ("project_key", "project", "project_slug", "slug", "project_id"):
            value = result.get(key)
    ...
    *,
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            project_lookup_value: Optional[Union[str, int]] = project_value
            recorded_agent_value = agent_value
            ...
            try:
                ...
                if recorded_agent_value is None:
                    maybe_agent = _extract_agent_from_result(result)
                    if maybe_agent:
                        recorded_agent_value = maybe_agent
                if project_lookup_value is None:
                    maybe_project = _extract_project_from_result(result)
                    if maybe_project is not None:
                        project_lookup_value = maybe_project
                        if project_value is None:
                            project_value = (
                                maybe_project
                                if isinstance(maybe_project, str)
                                else str(maybe_project)
                            )
                if project_lookup_value is not None and recorded_agent_value:
                    alias = await _resolve_agent_alias(
                        project_lookup_value, recorded_agent_value
                    )
                    if alias:
                        recorded_agent_value = alias
                ...
                _record_recent(tool_name, project_value, recorded_agent_value)
    ...
    if base_path.exists():
        ...
        for rel_name in _PROJECT_PROFILE_FILENAMES:
            ...
            if (
                ...
                continue
            preview = await _read_file_preview(
                candidate, max_chars=_PROJECT_PROFILE_PER_FILE_CHARS
            )
            if not preview:
                continue
...
def _heuristic_project_similarity(
    ...
    if project_a.human_key == project_b.human_key:
        return (
            0.0,
            "ERROR: Identical human_key - these are the SAME project, not siblings",
        )

    slug_ratio = SequenceMatcher(None, project_a.slug, project_b.slug).ratio()
    human_ratio = SequenceMatcher(
        None, project_a.human_key, project_b.human_key
    ).ratio()
    shared_prefix = 0.0
    try:
    ...
    if not reasons:
        reasons.append(
            "Heuristic comparison found limited overlap; treating as weak relation"
        )
...
async def _score_project_pair(
    ...
    settings = get_settings()
    heuristic_score, heuristic_reason = _heuristic_project_similarity(
        project_a, project_b
    )
    ...
    try:
        completion = await complete_system_user(
...
async def refresh_project_sibling_suggestions(
    *, max_pairs: int = _PROJECT_SIBLING_REFRESH_LIMIT
) -> None:
    await ensure_schema()
    async with get_session() as session:
        ...
        existing_rows = (
            (await session.execute(select(ProjectSiblingSuggestion))).scalars().all()
        )
        existing_map: dict[tuple[int, int], ProjectSiblingSuggestion] = {}
        for suggestion in existing_rows:
            pair = _canonical_project_pair(
                suggestion.project_a_id, suggestion.project_b_id
            )
            ...
            for project_b in projects[idx + 1 :]:
                ...
                if suggestion is None:
                    ...
                    if eval_ts is not None:
                        if (
                            eval_ts.tzinfo is None
                            or eval_ts.tzinfo.utcoffset(eval_ts) is None
                        ):
        ...
        for project_a, project_b, suggestion in to_evaluate[:max_pairs]:
            profile_a = await _build_project_profile(
                project_a, agent_map.get(project_a.id or -1, [])
            )
            profile_b = await _build_project_profile(
                project_b, agent_map.get(project_b.id or -1, [])
            )
            score, rationale = await _score_project_pair(
                project_a, profile_a, project_b, profile_b
            )
...
async def get_project_sibling_data() -> dict[int, dict[str, list[dict[str, Any]]]]:
    ...
    async with get_session() as session:
        ...
        result_map: dict[int, dict[str, list[dict[str, Any]]]] = defaultdict(
            lambda: {"confirmed": [], "suggested": []}
        )
        ...
        for row in rows.fetchall():
            ...
            for current, other in ((a_info, b_info), (b_info, a_info)):
                ...
                if entry["status"] == "confirmed":
                    ...
                    and float(entry_base["score"])
                    >= _PROJECT_SIBLING_MIN_SUGGESTION_SCORE
                ):
...
async def update_project_sibling_status(
    project_id: int, other_id: int, status: str
) -> dict[str, Any]:
    ...
    async with get_session() as session:
        ...
        if suggestion is None:
            ...
            projects = [
                proj for proj in (project_a_obj, project_b_obj) if proj is not None
            ]
            if len(projects) != 2:
            ...
            for proj_id, name in agents_rows.fetchall():
                agent_map[int(proj_id)].append(name)
            profile_a = await _build_project_profile(
                project_map[pair[0]], agent_map.get(pair[0], [])
            )
            profile_b = await _build_project_profile(
                project_map[pair[1]], agent_map.get(pair[1], [])
            )
            score, rationale = await _score_project_pair(
        ...
        return {
            ...
            "evaluated_ts": str(suggestion.evaluated_ts)
    ...
    async with get_session() as session:
        result = await session.execute(
            select(Agent.id).where(
                Agent.project_id == project.id, func.lower(Agent.name) == name.lower()
            )
        )
        return result.first() is not None


async def _compute_agent_alias(project_id: int, agent_name: str) -> Optional[str]:
    normalized_target = agent_name.strip().lower()
    async with get_session() as session:
        result = await session.execute(
            select(Agent.name)
    ...
    for index, existing in enumerate(names):
        if existing.lower() == normalized_target:
            if index < len(AGENT_ALIAS_SEQUENCE):
                return AGENT_ALIAS_SEQUENCE[index]
            return f"Agent{index + 1}"
    return None


async def _resolve_agent_alias(
    project_identifier: Union[str, int, None], agent_name: Optional[str]
) -> Optional[str]:
    if not project_identifier or not agent_name:
        return None
    try:
        if isinstance(project_identifier, int):
            async with get_session() as session:
                project = await session.get(Project, project_identifier)
                if project is None:
                    return None
        else:
            project = await _get_project_by_identifier(str(project_identifier))
    except Exception:
        return None
    if project.id is None:
        return None
    return await _compute_agent_alias(project.id, agent_name)
...
async def _generate_unique_agent_name(
    ...
    async def available(candidate: str) -> bool:
        return (
            not await _agent_name_exists(project, candidate)
            and not (archive.root / "agents" / candidate).exists()
        )
    ...
    async with get_session() as session:
        ...
        for recipient, kind in recipients:
            entry = MessageRecipient(
                message_id=message.id, agent_id=recipient.id, kind=kind
            )
            session.add(entry)
...
async def _create_file_reservation(
    ...
    if project.id is None or agent.id is None:
        raise ValueError(
            "Project and agent must have ids before creating file_reservations."
        )
...
def _file_reservations_conflict(
    ...
    def _expand_dir_star(p: str) -> str:
        if p.endswith("/*"):
            return (
                p[:-1] + "*"
            )  # "src/*" -> "src/**"-like breadth for fnmatchcase approximation
        return p
...
def _file_reservations_patterns_overlap(
    paths_a: Sequence[str], paths_b: Sequence[str]
) -> bool:
    for pa in paths_a:
        for pb in paths_b:
...
def _canonical_relpath_for_message(
    project: Project, message: Message, archive
) -> str | None:
...
async def _commit_info_for_message(
    settings: Settings, project: Project, message: Message
) -> dict[str, Any] | None:
    ...
    def _lookup():
        ...
        data: dict[str, Any] = {
            ...
            "authored_ts": _iso(
                datetime.fromtimestamp(commit.authored_date, tz=timezone.utc)
            ),
        }
        try:
...
def _summarize_messages(messages: Sequence[tuple[Message, str]]) -> dict[str, Any]:
    ...
    for message, sender_name in messages:
        ...
        for line in message.body_md.splitlines():
            ...
            if stripped.startswith(("-", "*", "+")) or stripped[:2] in {
                "1.",
                "2.",
                "3.",
                "4.",
    ...
    async with get_session() as session:
        result = await session.execute(
            select(Message).where(
                Message.project_id == project.id, Message.id == message_id
            )
        )
        message = result.scalars().first()
        if not message:
            raise NoResultFound(
                f"Message '{message_id}' not found for project '{project.human_key}'."
            )
        return message
...
async def _get_agent_by_id(project: Project, agent_id: int) -> Agent:
    ...
    async with get_session() as session:
        ...
        if not agent:
            raise NoResultFound(
                f"Agent id '{agent_id}' not found for project '{project.human_key}'."
            )
        return agent
...
def build_mcp_server() -> FastMCP:
    ...
    async def _deliver_message(
        ...
        recipient_records: list[tuple[Agent, str]] = [
            (agent, "to") for agent in to_agents
        ]
        ...
        async with AsyncFileLock(archive.lock_path):
            ...
            if settings.file_reservations_enforcement_enabled:
                ...
                candidate_surfaces.append(
                    f"agents/{sender.name}/outbox/{y_dir}/{m_dir}/*.md"
                )
                for r in to_agents + cc_agents + bcc_agents:
                    candidate_surfaces.append(
                        f"agents/{r.name}/inbox/{y_dir}/{m_dir}/*.md"
                    )
                ...
                for surface in candidate_surfaces:
                    for (
                        ...
                        holder_name,
                    ) in active_file_reservations:
                        if _file_reservations_conflict(
                            file_reservation_record, surface, True, sender
                        ):
                            conflicts.append(
                                {
                                    "surface": surface,
                                    ...
                                    "expires_ts": _iso(
                                        file_reservation_record.expires_ts
                                    ),
                                }
                            )
                if conflicts:
    ...
    async def ensure_project(ctx: Context, human_key: str) -> dict[str, Any]:
        ...
        # Windows Path.is_absolute() treats "/foo" as relative, so allow POSIX-style roots too.
        human_path = Path(human_key)
        if not human_key.startswith("/") and not human_path.is_absolute():
            raise ValueError(
    ...
    async def register_agent(
        ...
        agent = await _get_or_create_agent(
            project, name, program, model, task_description, settings
        )
    ...
    async def whois(
        ...
        await ctx.info(
            f"whois for '{agent_name}' in '{project.human_key}' returned {len(recent)} commits"
        )
        return profile
    ...
    async def create_agent_identity(
        ...
        agent = await _create_agent_record(
            project, unique_name, program, model, task_description
        )
    ...
    async def send_message(
        ...
        if cc is not None and not isinstance(cc, list):
            await ctx.error(
                "INVALID_ARGUMENT: cc must be a list of strings or a single string."
            )
            raise ToolExecutionError(
                "INVALID_ARGUMENT",
        ...
        if bcc is not None and not isinstance(bcc, list):
            await ctx.error(
                "INVALID_ARGUMENT: bcc must be a list of strings or a single string."
            )
            raise ToolExecutionError(
                "INVALID_ARGUMENT",
        ...
        if bcc is not None and any(not isinstance(x, str) for x in bcc):
            await ctx.error(
                "INVALID_ARGUMENT: bcc items must be strings (agent names)."
            )
            raise ToolExecutionError(
                "INVALID_ARGUMENT",
        ...
        if settings_local.contact_enforcement_enabled and not ack_required:
            ...
            try:
                async with get_session() as s2:
                    file_reservation_rows = await s2.execute(
                        ...
                        .where(
                            FileReservation.project_id == project.id,
                            cast(Any, FileReservation.released_ts).is_(None),
                            FileReservation.expires_ts > now_utc,
                        )
                    )
                    name_to_file_reservations: dict[str, list[str]] = {}
                    for c, nm in file_reservation_rows.all():
                        name_to_file_reservations.setdefault(nm, []).append(
                            c.path_pattern
                        )
                sender_file_reservations = name_to_file_reservations.get(
                    sender.name, []
                )
                for nm in to + (cc or []) + (bcc or []):
                    ...
                    if (
                        ...
                        and their
                        and _file_reservations_patterns_overlap(
                            sender_file_reservations, their
                        )
                    ):
            ...
            async with get_session() as s3:
                for nm in to + (cc or []) + (bcc or []):
                    ...
                    ttl = timedelta(
                        seconds=int(settings_local.contact_auto_ttl_seconds)
                    )
                    recent_ok = False
                    try:
            ...
            if blocked_recipients:
                ...
                effective_auto_contact: bool = bool(
                    ...
                    or getattr(
                        settings_local, "messaging_auto_handshake_on_block", True
                    )
                )
                if effective_auto_contact:
                    try:
                        ...
                        handshake = cast(
                            FunctionTool, cast(Any, macro_contact_handshake)
                        )
                                ...
                                await handshake.run(
                                    {
                                        ...
                                        "auto_accept": True,
                                        "ttl_seconds": int(
                                            settings_local.contact_auto_ttl_seconds
                                        ),
                                    }
                                )
                                attempted.append(nm)
        ...
        async with get_session() as sx:
            ...
            existing = await sx.execute(
                select(Agent.name).where(Agent.project_id == project.id)
            )
            ...
            for row in existing.fetchall():
                ...
                if not canonical_name:
                    continue
                sanitized_canonical = (
                    sanitize_agent_name(canonical_name) or canonical_name
                )
                ...
                for raw in name_list:
                    ...
                    if candidate.startswith("project:") and "#" in candidate:
                        ...
                        try:
                            ...
                            target_project_override = await _get_project_by_identifier(
                                slug_part.strip()
                            )
                            target_project_label = (
                                target_project_override.human_key
                                or target_project_override.slug
                            )
                        ...
                        if name_part.strip() and project_part.strip():
                            try:
                                target_project_override = (
                                    await _get_project_by_identifier(
                                        project_part.strip()
                                    )
                                )
                                target_project_label = (
                    ...
                    display_value, key_candidates, canonical = _normalize(
                        agent_fragment
                    )
                    ...
                    if not explicit_override and sender_candidate_keys.intersection(
                        key_candidates
                    ):
                        if kind == "to":
                    ...
                    if rec:
                        ...
                        pol = (
                            getattr(target_agent, "contact_policy", "auto") or "auto"
                        ).lower()
                        if pol == "block_all":
                            await ctx.error(
                                "CONTACT_BLOCKED: Recipient is not accepting messages."
                    ...
                    if explicit_override:
                        label = target_project_label or "(unknown project)"
                        unknown_external[label].append(
                            display_value or candidate.strip() or candidate
                        )
                    else:
                        unknown_local.add(
                            display_value or candidate.strip() or candidate
            ...
            if unknown_local or unknown_external:
                                    ...
                                    await handshake.run(
                                        {
                                            ...
                                            "requester": sender.name,
                                            "target": nm,
                                            "to_project": target_proj.human_key
                                            or target_proj.slug,
                                            "reason": "auto-handshake by send_message",
                                            "auto_accept": True,
                                            "ttl_seconds": int(
                        ...
                        if attempted_external:
                            ...
                            try:
                                async with get_session() as scheck:
                                    for label, names in list(unknown_external.items()):
                                        try:
                                            tproj = await _get_project_by_identifier(
                                                label
                                            )
                                        except Exception:
                                            continue
                                        ...
                                        for nm in list(names):
                                            ...
                                            rows = await scheck.execute(
                                                ...
                                                .join(
                                                    Project,
                                                    Project.id
                                                    == AgentLink.b_project_id,
                                                )
                                                .join(
                                                    Agent,
                                                    Agent.id == AgentLink.b_agent_id,
                                                )
                                                .where(
                                                    AgentLink.a_project_id
                                                    == project.id,
                                                    AgentLink.a_agent_id == sender.id,
                                                    AgentLink.status == "approved",
                                                    Project.id == tproj.id,
                ...
                still_unknown = bool(unknown_local) or any(
                    v for v in unknown_external.values()
                )
                ...
                if still_unknown:
                    ...
                    try:
                        ...
                        for label, names in (
                            data_payload.get("unknown_external", {}) or {}
                        ).items():
                            for nm in names[:5]:
                                suggestions.append(
                                    {
        ...
        if len(deliveries) == 1:
            ...
            if isinstance(maybe_payload, dict) and isinstance(
                maybe_payload.get("error"), dict
            ):
    ...
    async def reply_message(
        ...
        if subject_prefix_clean and base_subject.lower().startswith(
            subject_prefix_clean.lower()
        ):
        ...
        for _pid, group in external.items():
            ...
            try:
                ...
                deliveries.append(
                    {"project": target_project.human_key, "payload": payload_ext}
                )
            except Exception:
                continue
    ...
    async def respond_contact(
        ...
        a_project = (
            project
            if not from_project
            else await _get_project_by_identifier(from_project)
        )
        ...
        async with get_session() as s:
            ...
            await s.commit()
        await ctx.info(
            f"Contact {'approved' if accept else 'denied'}: {from_agent} -> {to_agent}"
        )
        return {
            "from": from_agent,
            "to": to_agent,
    ...
    async def list_contacts(
        ctx: Context, project_key: str, agent_name: str
    ) -> list[dict[str, Any]]:
    ...
    async def set_contact_policy(
        ctx: Context, project_key: str, agent_name: str, policy: str
    ) -> dict[str, Any]:
    ...
    async def fetch_inbox(
        ...
        try:
            ...
            items = await _list_inbox(
                project, agent, limit, urgent_only, include_bodies, since_ts
            )
            await ctx.info(
                f"Fetched {len(items)} messages for '{agent.name}'. urgent_only={urgent_only}"
            )
            return items
        except Exception as exc:
    ...
    async def macro_start_session(
        ...
        project = await _ensure_project(human_key)
        agent = await _get_or_create_agent(
            project, agent_name, program, model, task_description, settings
        )
        ...
        if file_reservation_paths:
            ...
            _file_reservation_tool = cast(
                FunctionTool, await mcp.get_tool("file_reservation_paths")
            )
            ...
            file_reservations_result = cast(
                dict[str, Any], _file_reservation_run.structured_content or {}
            )
    ...
    async def macro_file_reservation_cycle(
        ...
        file_reservations_result = cast(
            dict[str, Any], file_reservations_tool_result.structured_content or {}
        )
        ...
        if auto_release:
            ...
            release_result = cast(
                dict[str, Any], release_tool_result.structured_content or {}
            )
    ...
    async def macro_contact_handshake(
            ...
            try:
                ...
                if not real_target and project.id is not None:
                    ...
                    if real_requester and len(names2) == 2 and real_requester in names2:
                        real_target = next(
                            (n for n in names2 if n != real_requester), real_target
                        )
            except Exception:
                pass
        if not real_requester or not real_target:
        ...
        request_result = cast(
            dict[str, Any], request_tool_result.structured_content or {}
        )
        ...
        if auto_accept:
            ...
            response_result = cast(
                dict[str, Any], respond_tool_result.structured_content or {}
            )
        ...
        if welcome_subject and welcome_body and not target_project_key:
            try:
                ...
                welcome_result = cast(
                    dict[str, Any], send_tool_result.structured_content or {}
                )
            except ToolExecutionError as exc:
                # surface but do not abort handshake
                await ctx.debug(
                    f"macro_contact_handshake failed to send welcome: {exc}"
                )
    ...
    async def search_messages(
        ...
        if get_settings().tools_log_enabled:
            try:
                ...
                cons.print(
                    Panel(body, title="tool: search_messages", border_style="green")
                )
            except Exception:
                pass
        if project.id is None:
        ...
        await ctx.info(
            f"Search '{query}' returned {len(rows)} messages for project '{project.human_key}'."
        )
        if get_settings().tools_log_enabled:
            try:
                import importlib as _imp
                ...
                Console().print(
                    Panel(
                        f"results={len(rows)}",
                        title="tool: search_messages — done",
                        border_style="green",
                    )
                )
            except Exception:
    ...
    async def summarize_threads(
        ...
        async with get_session() as session:
            for tid in thread_ids:
                ...
                for m in summary.get("mentions", []):
                    ...
                    if not name:
                        continue
                    all_mentions[name] = all_mentions.get(name, 0) + int(
                        m.get("count", 0) or 0
                    )
                    ...
                    if threads_payload:
                        ...
                        for item in thread_summaries:
                            ...
                            if tid in mapping:
                                ...
                                revised_threads.append(
                                    {"thread_id": item["thread_id"], "summary": s}
                                )
                            else:
        ...
        await ctx.info(
            f"Summarized {len(thread_ids)} thread(s) for project '{project.human_key}'."
        )
    ...
    async def install_precommit_guard(
        ...
        await ctx.info(
            f"Installed pre-commit guard for project '{project.human_key}' at {hook_path}."
        )
    ...
    async def uninstall_precommit_guard(
        ...
        if get_settings().tools_log_enabled:
            try:
                ...
                Console().print(
                    Panel.fit(
                        f"repo={code_repo_path}",
                        title="tool: uninstall_precommit_guard",
                        border_style="green",
                    )
                )
            except Exception:
        ...
        if removed:
            await ctx.info(
                f"Removed pre-commit guard at {repo_path / '.git/hooks/pre-commit'}."
            )
        else:
            await ctx.info(
                f"No pre-commit guard to remove at {repo_path / '.git/hooks/pre-commit'}."
            )
    ...
    async def file_reservation_paths(
        ...
        async with AsyncFileLock(archive.lock_path):
            for path in paths:
                ...
                for file_reservation_record, holder_name in existing_reservations:
                    if _file_reservations_conflict(
                        file_reservation_record, path, exclusive, agent
                    ):
        ...
        await ctx.info(
            f"Issued {len(granted)} file_reservations for '{agent.name}'. Conflicts: {len(conflicts)}"
        )
    ...
    async def release_file_reservations_tool(
        ...
        if get_settings().tools_log_enabled:
            try:
                ...
                Console().print(
                    Panel.fit(
                        "\n".join(details),
                        title="tool: release_file_reservations",
                        border_style="green",
                    )
                )
            except Exception:
        ...
        try:
            ...
            if project.id is None or agent.id is None:
                raise ValueError(
                    "Project and agent must have ids before releasing file_reservations."
                )
            await ensure_schema()
            now = datetime.now(timezone.utc)
            async with get_session() as session:
    ...
    async def renew_file_reservations(
        ...
        if get_settings().tools_log_enabled:
            try:
                ...
                Console().print(
                    Panel.fit(
                        "\n".join(meta),
                        title="tool: renew_file_reservations",
                        border_style="green",
                    )
                )
            except Exception:
        ...
        if project.id is None or agent.id is None:
            raise ValueError(
                "Project and agent must have ids before renewing file_reservations."
            )
        await ensure_schema()
        now = datetime.now(timezone.utc)
        ...
        await ctx.info(
            f"Renewed {len(updated)} file_reservation(s) for '{agent.name}'."
        )
                ...
                "tools": [
                    {
                        ...
                        "usage_examples": [
                            {"hint": "Pre-flight", "sample": "health_check()"}
                        ],
                    },
                    {
                        "name": "ensure_project",
    ...
    @mcp.resource(
        "resource://tooling/capabilities/{agent}{?project}", mime_type="application/json"
    )
    def tooling_capabilities_resource(
        agent: str, project: Optional[str] = None
    ) -> dict[str, Any]:
    ...
    async def projects_resource() -> list[dict[str, Any]]:
        ...
        async with get_session() as session:
            result = await session.execute(
                select(Project).order_by(asc(Project.created_at))
            )
            ...
            filtered = [
                p
                for p in projects
                if not (_is_ignored(p.slug) or _is_ignored(p.human_key))
            ]
        ...
        async with get_session() as session:
            ...
            unread_counts_stmt = (
                ...
                .where(
                    ...
                    cast(Any, MessageRecipient.agent_id).in_(
            ...
            unread_counts_map = {
                row.agent_id: row.unread_count for row in unread_counts_result
            }
    ...
    async def file_reservations_resource(
        slug: str, active_only: bool = False
    ) -> list[dict[str, Any]]:
        """
        ...
        if project.id is None:
            raise ValueError(
                "Project must have an id before listing file_reservations."
            )
        await _expire_stale_file_reservations(project.id)
        async with get_session() as session:
            stmt = (
        ...
        return [
            {
                ...
                "created_ts": _iso(file_reservation.created_ts),
                "expires_ts": _iso(file_reservation.expires_ts),
                "released_ts": _iso(file_reservation.released_ts)
                if file_reservation.released_ts
                else None,
            }
    ...
    @mcp.resource(
        "resource://message/{message_id}{?project}", mime_type="application/json"
    )
    async def message_resource(
        message_id: str, project: Optional[str] = None
    ) -> dict[str, Any]:
        """
    ...
    async def inbox_resource(
        ...
        messages = await _list_inbox(
            project_obj, agent_obj, limit, urgent_only, include_bodies, since_ts
        )
        ...
        for item in messages:
            try:
                ...
                commit_info = await _commit_info_for_message(
                    settings, project_obj, msg_obj
                )
                if commit_info:
            ...
            for item in items:
                result = await session.execute(
                    select(MessageRecipient.read_ts).where(
                        MessageRecipient.message_id == item["id"],
    ...
    async def acks_stale_view(
        ...
        """
        logger.debug("acks_stale_view input agent=%s project=%s ttl_seconds=%r limit=%s", agent, project, ttl_seconds, limit)
        query_args = _query_params_from_context(ctx)
        # Parse query embedded in agent path if present
        if "?" in agent:
            name_part, _, qs = agent.partition("?")
            agent = name_part
            try:
                ...
                pass
        if project is None:
            project_candidate = query_args.get("project")
            if project_candidate:
                project = project_candidate
        if "ttl_seconds" in query_args:
            with suppress(Exception):
                ttl_seconds = int(query_args["ttl_seconds"])
        ...
        if not isinstance(limit, int):
            with suppress(Exception):
                limit = int(limit)
        limit = max(1, limit)
        logger.debug("acks_stale_view post-parse agent=%s project=%s ttl_seconds=%r limit=%s", agent, project, ttl_seconds, limit)
        ...
        ttl_raw = (
            ...
            else get_settings().ack_ttl_seconds
        )
        ttl = max(0, ttl_raw)
        logger.debug("acks_stale_view resolved ttl=%s", ttl)
        now = datetime.now(timezone.utc)
        out: list[dict[str, Any]] = []
        async with get_session() as session:
            rows = await session.execute(
    ...
    async def ack_overdue_view(
        ...
        if project is None:
            ...
            if project_candidate:
                project = project_candidate
        if "ttl_minutes" in query_args:
            with suppress(Exception):
                ttl_minutes = int(query_args["ttl_minutes"])
        if "limit" in query_args:
            with suppress(Exception):
                limit = int(query_args["limit"])
    ...
    @mcp.resource(
        "resource://mailbox/{agent}{?project,limit}", mime_type="application/json"
    )
    async def mailbox_resource(
        ...
        for item in items:
            ...
            if commits_index:
                commit_meta = next(
                    iter(commits_index.values())
                )  # provide at least one recent reference
            payload = dict(item)
    ...
    async def mailbox_with_commits_resource(
        ...
        if project is None:
            ...
            if len(projects) == 1:
                ...
                raise ValueError(
                    "project parameter is required for mailbox-with-commits resource"
                )
        else:
            project_obj = await _get_project_by_identifier(project)
        agent_obj = await _get_agent(project_obj, agent)
    ...
    async def outbox_resource(
            ...
            try:
                ...
                if parsed.get("limit"):
                    ...
                    with suppress(Exception):
                        limit = int(parsed["limit"][0])
                if parsed.get("include_bodies"):
                    include_bodies = parsed["include_bodies"][0].lower() in {
                        "1",
                        "true",
                        "t",
                        "yes",
        ...
        project_obj = await _get_project_by_identifier(project)
        agent_obj = await _get_agent(project_obj, agent)
        items = await _list_outbox(
            project_obj, agent_obj, limit, include_bodies, since_ts
        )
        enriched: list[dict[str, Any]] = []
        for item in items:
            try:
