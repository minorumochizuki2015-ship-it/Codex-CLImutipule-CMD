...
async def complete_system_user(system: str, user: str, *, model: Optional[str] = None, temperature: Optional[float] = None, max_tokens: Optional[int] = None) -> LlmOutput:
    ...
    settings = get_settings()
    # Guard: disable completions when LLM is disabled via settings
    if not settings.llm.enabled:
        _logger.info("llm.disabled", reason="LLM_ENABLED=false")
        return LlmOutput(content="", model="disabled", provider=None)
